...
COORD="xcnc25"
WORKERS="xcnc26;xcnc27;xcnc28;xcnc29"
CLUSTER_IPS="xcnc25,xcnc26,xcnc27,xcnc28,xcnc29"
NODE=$(hostname)
...
if $deploy_citus; then
    ...
    srun --nodes=5 --ntasks=5 --cpus-per-task=4 --nodelist=xcnc[25-29] ${SCRIPTSDIR}/deploy-citus.sh ${COORD} ${WORKERS[@]} &
...
if $load_data; then
    ...
    srun --nodes=5 --ntasks=5 --cpus-per-task=4 --nodelist=xcnc[25-29] ${SCRIPTSDIR}/load-citus-data.sh ${COORD} &
fi
...
if $exec_transactions; then
    ...
    srun --nodes=1 --ntasks=1 --cpus-per-task=2 --nodelist=xcnc25 python3 ${SCRIPTSDIR}/generate_metrics.py ${CLUSTER_IPS} ${RESULTSDIR}
fi
